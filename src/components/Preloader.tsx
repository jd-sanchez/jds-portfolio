    ...
    () => {
      ...
      mm.add(
        ...
        (context) => {
          ...
          const split = new SplitText("[data-preloader-chars]", {
            type: "chars",
          ...
          // Measure where the small nav logo lives so the big loader word can
          // land straight into it. We tween `font-size` (on the word) plus a
          // translate (on the group) instead of `scale` — scaling text via
          // transform rasterizes it once and resamples, which reads as a soft
          // blur while it moves. Shrinking the real font size keeps every
          // frame crisply re-rendered.
          ...
          tl.set(group, { transformOrigin: "0% 0%" })
            .set(split.chars, { opacity: 0, yPercent: 120, rotateX: -60 })
          ...
          if (morph) {
            tl.addLabel("land")
              ...
              // finished shrinking into place, so the landed logo matches the
              // real nav mark exactly before the page fades in behind it —
              // no mismatched glyph popping in mid-fade.
              .to(
            ...
            if (revealOrigin) {
              // Let the landed logo sit for a beat — an instant cut the moment
              // it settles reads as abrupt — then open the page through an
              // iris centered on the logo instead of a flat fade, so the
              // reveal itself feels like it's emanating from where it landed.
